package com.remat.domain.material.controller.docs;
...
public interface MaterialApi {
    ...
    ApiResponse<List<MaterialResDTO.ListDTO>> getMaterials(
            @Parameter(description = "카테고리 (미입력 시 전체 조회)", example = "콘크리트") String categoryName,
            @Parameter(description = "자재 상태 (BEST / GOOD / NORMAL)", example = "BEST") MaterialCondition materialCondition,
            @Parameter(description = "거래 유형 (SALE / RENTAL)", example = "SALE") TransactionType transactionType,
            @Parameter(description = "지역 (INCHEON, SEOUL 등)", example = "INCHEON") Region region
    );
    ...
    ApiResponse<List<MaterialResDTO.MyListDTO>> getMyMaterials(
    ...
    @ApiAuthErrorResponse
    ApiResponse<MaterialResDTO.ImageUploadDTO> uploadImage(
    ...
    @ApiAuthErrorResponse
    ApiResponse<Void> createMaterial(
    ...
    ApiResponse<MaterialResDTO.DetailDTO> getMaterialDetail(
            @Parameter(description = "자재 ID", required = true, example = "1") Long materialId
    );
    ...
    ApiResponse<MaterialResDTO.CategoryListDTO> getCategories();
}
