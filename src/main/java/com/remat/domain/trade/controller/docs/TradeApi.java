package com.remat.domain.trade.controller.docs;
...
public interface TradeApi {
    ...
    ApiResponse<Void> createTradeRequest(
            TradeReqDTO.CreateDTO reqDto,
    ...
    @ApiAuthErrorResponse
    ApiResponse<List<TradeResDTO.ReceivedRequestDTO>> getReceivedTradeRequests(
            @Parameter(hidden = true) UserDetailsImpl userDetails
    );
    ...
    @ApiAuthErrorResponse
    ApiResponse<List<TradeResDTO.SentRequestDTO>> getSentTradeRequests(
            @Parameter(hidden = true) UserDetailsImpl userDetails
    );
    ...
    @ApiAuthErrorResponse
    ApiResponse<Void> approveTradeRequest(
            @Parameter(description = "거래 요청 ID", required = true, example = "1") Long tradeRequestId,
            TradeReqDTO.ApproveDTO reqDto,
            @Parameter(hidden = true) UserDetailsImpl userDetails
    );
    ...
    @ApiAuthErrorResponse
    ApiResponse<List<TradeResDTO.PurchasedTradeDTO>> getPurchasedTrades(
            @Parameter(hidden = true) UserDetailsImpl userDetails
    );
    ...
    @ApiAuthErrorResponse
    ApiResponse<List<TradeResDTO.SoldTradeDTO>> getSoldTrades(
            @Parameter(hidden = true) UserDetailsImpl userDetails
    );
    ...
    @ApiAuthErrorResponse
    ApiResponse<Void> createTradeReview(
            @Parameter(description = "거래 ID", required = true, example = "1") Long tradeId,
            TradeReqDTO.ReviewCreateDTO reqDto,
            @Parameter(hidden = true) UserDetailsImpl userDetails
    );
}
